defmodule Core.PluginWeather do
  ...
  def init(_state) do
    ...
    token = File.read("apps/core/lib/plugins/weather/weather_token.sec")
    {appid, msg} = case token do
                     {:ok, appid} -> {appid, "[INFO] weather token successfully read"}
                     _ -> {"", "[WARN] weather token not found"}
                   end
    IO.puts(msg)
