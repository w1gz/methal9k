defmodule Hal.Dispatcher do
  ...
  alias Hal.Plugin.Spotify, as: Spotify
  ...
  defp check_out_the_big_brain_on_brett(infos) do
    ...
    case cmd do
      ...
      ".spotify"    -> spotify_search(rm_cmd(infos.msg, cmd), infos)
      _             -> emojis(cmd, infos)
    end
  end
  ...
  defp help_cmd(infos) do
    ...
    plugins_answers = Enum.map(plugins, fn(module) ->
      Pool.transaction(module, fn(pid) ->
        module.usage(pid)
      end)
    end)
  ...
  defp weather(params, infos) do
    case params do
      [] ->
        ...
        Handler.terminate(infos)
      ["hourly" | arg2] ->
        Pool.transaction(Weather, fn(pid) ->
  ...
  defp get_bouncer(infos) do
    Pool.transaction(Bouncer, fn(pid) ->
  ...
  defp manage_quote(quoted_action, infos) do
    Pool.transaction(Quote, &(Quote.manage_quote(&1, quoted_action, infos)))
  end

  defp web_search(params, infos) do
    Pool.transaction(Web, &(Web.search(&1, params, infos)))
  end

  defp spotify_search(params, infos) do
    Pool.transaction(Spotify, &(Spotify.search(&1, params, infos)))
  end
